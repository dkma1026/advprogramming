//Diana Ma, dkma@ucsc.edu
//CMPS109 Spring15 Mackey
//Assignment: asg5- client/server and sockets
...
void reply_get(accepted_socket& client_sock, cix_header& header) {
    ...
    log << "recieved header in get " << header << endl;
   //get file size
    stringstream sstr;
    if (not fs.good()) {
        log << "get opening file: " << header.filename << ": failed" << endl;
        header.command = CIX_NAK;
        send_packet (client_sock, &header, sizeof header);
    }
    sstr << fs.rdbuf();
    ...
    header.nbytes = data.size(header.filename);
    send_packet (client_sock, &header, sizeof header);
...
void reply_put(accepted_socket& client_sock, cix_header& header) {
    ...
    string put_output;
    if (not fs.good()) {
        log << "put: failed to open file for writing";
        header.command = CIX_NAK;
        send_packet (client_sock, &header, sizeof header);
        return;
    }
    ...
    log << "received " << header.nbytes << " bytes of data" << endl;
...
void reply_rm(accepted_socket& client_sock, cix_header& header) {
    int retval = unlink(header.filename);
    if (retval != 0) {
        log << "failed to delete file( " << strerror (errno) << endl;
        header.command = CIX_NAK;
        header.nbytes = errno;
        send_packet (client_sock, &header, sizeof header);
        return;
    ...
    header.command = CIX_ACK;
    header.nbytes = 0;
    log << "recieved header in rm " << header << endl;
    send_packet (client_sock, &header, sizeof header);
}
